// Package fullsmoke exercises the complete typed Management SDK surface used
// by the runnable Production demo.
package fullsmoke
...
// Config configures a complete Management SDK smoke run.
type Config struct {
	Management *management.Client
	OAuth      *oauth2.Client
	Writes     bool
...
// Run executes the complete typed Management SDK smoke surface.
func Run(ctx context.Context, cfg Config) error {
	if cfg.Management == nil {
		return errors.New("full smoke Management client is required")
	}
	if cfg.OAuth == nil {
		return errors.New("full smoke OAuth client is required")
	}
	...
	if oauthEnabled(accessToken, clientID, clientSecret) && accessToken == "" {
		token, err := cfg.OAuth.ClientCredentials(ctx, oauth2.ClientCredentialsRequest{
			ClientID:     clientID,
	...
	r.print()
	if !r.complete() {
		return errors.New("Management SDK full smoke is incomplete")
	}
	return nil
}
...
func (r *runner) runWorkspaceAccess() {
	...
	r.call("list workspace members", "GET /v1/workspaces/:workspace_id/members", func() error {
		...
		if userID := firstRegisteredMemberUserID(members.Items); userID > 0 {
			...
			r.skip("get workspace member", "GET /v1/workspaces/:workspace_id/members/:member_id", "members list returned no registered member with a positive user ID")
			r.skip("get workspace member permissions", "GET /v1/workspaces/:workspace_id/members/:member_id/permissions", "members list returned no registered member with a positive user ID")
		}
		return nil
	})
...
func (r *runner) runGateway() {
	...
	r.call("list models", "GET /v1/gateway/models", func() error {
		...
		if len(models.Items) > 0 {
			modelName = models.Items[0].ModelID
			if modelName == "" {
				modelName = models.Items[0].ID
			}
		}
		if modelName == "" {
			modelName = strings.TrimSpace(os.Getenv("OWLVIGIL_SMOKE_MODEL_ID"))
		}
		return nil
	})
	if modelName != "" {
	...
	if modelName != "" {
		r.call("preview route", "POST /v1/gateway/routes/preview", func() error {
			_, _, err := r.management.PreviewRoute(r.ctx, &management.PreviewRouteRequest{WorkspaceID: r.workspaceID, Model: modelName})
			return err
		})
	} else {
...
func (r *runner) runFinancial() {
	...
	if !r.writes {
		...
		originalLimits := append([]management.SpendingLimit(nil), limits.Items...)
		...
		r.writeSkipExpected("update user spending limit", "PATCH /v1/workspaces/:workspace_id/governance/financial/spending-limits/users/:user_id", "financial write endpoint not exercised in smoke", teamBudgetFeatureGate, func() error {
			...
			cleanupCtx, cancel := r.cleanupContext()
			defer cancel()
			_, _, err = r.management.UpdateSpendingLimits(
				cleanupCtx,
				r.workspaceID,
				&management.UpdateSpendingLimitsRequest{Limits: originalLimits},
			)
			if err != nil {
				return fmt.Errorf("restore spending limits: %w", err)
			}
			return nil
		})
	}
	...
	if eventID != "" {
		...
		r.write("retry webhook event", "POST /v1/webhook-events/:event_id/retry", "webhook write endpoint not exercised in smoke", func() error {
			...
			if !strings.EqualFold(strings.TrimSpace(event.Status), "pending") {
				return fmt.Errorf(
					"retry webhook event status = %q, want pending",
					event.Status,
				)
			}
			return nil
...
func positiveLimitOrDefault(value float64) float64 {
...
func firstRegisteredMemberUserID(members []management.Member) int64 {
